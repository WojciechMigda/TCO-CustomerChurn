...
def read_train():
    ...
    df = pd.read_csv('../data/history.csv', na_values='*******',
#            nrows=100000
        )
...
def main():
    ...
    for colname in df:
        col = df[colname]
        has_nans = 'NaN' if col.isnull().values.any() else ''
        ...
        print('{:>30s} {:>8s} {:>3s} {:>8d} {:>8s} {:>8s} {}'.format(colname, col.dtype.name, has_nans, uniq_vals, str(min_), str(max_), obj_vals))
